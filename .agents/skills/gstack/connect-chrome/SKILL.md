...
description: |
  ...
  One command: connects Codex to a visible Chrome window where you can watch every
...
~/.Codex/skills/gstack/bin/gstack-timeline-log '{"skill":"connect-chrome","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Connect Codex to a visible Chrome window with the gstack extension auto-loaded.
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.Codex/skills/gstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.Codex/skills/gstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.Codex/skills/gstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.Codex/skills/gstack/extension"
...
commands appear in the activity feed. Every command Codex runs shows up here
in real time."
...
> snapshot and describe this page." A sidebar agent (a child Codex instance)
...
> it won't interfere with this Codex window.
