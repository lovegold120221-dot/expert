...
elif [ -d ".Codex/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".Codex/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".Codex/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.Codex/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.Codex/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.Codex/skills/gstack"
  fi
fi
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.Codex/skills/gstack/` when you're ready."
...
.Codex/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.Codex/skills/gstack/` when you're ready."
