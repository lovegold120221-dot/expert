...
> - `gemini-2.0-flash-live-preview-04-09` — Deprecated.
...
session.sendRealtimeInput({
  audio: { data: chunk.toString('base64'), mimeType: 'audio/pcm' }
});
```
...
- **Response modality** — Only `AUDIO` **or** `TEXT` per session, not both simultaneously. `AUDIO` is the correct choice for voice apps. Video output for avatars is handled externally (e.g., HeyGen) via function calling, not via native Live API video output.
...
- **`sendClientContent`** — Only use for seeding initial context history. For live text input during conversation, use `sendRealtimeInput({ text })` instead.
